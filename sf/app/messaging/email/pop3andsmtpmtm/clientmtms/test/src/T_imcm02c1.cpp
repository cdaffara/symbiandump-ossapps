...
//
// Description:
// Name of test harness: T_IMCM02C1
// Component: IMCM
// Owner: KP
...
// \MSG\IMCMTSRC\T_IMCM02C1_Entry_Structure.txt	<DRIVE>:\msgtest\IMCM\T_IMCM02C1_Entry_Structure.txt
...
// <DRIVE>:\msglogs\T_IMCM02C1\Entry_Structure.txt
// Output files produced by running test harness:
// <DRIVE>:\msglogs\T_IMCM02C1.<PLATFORM>.<VARIANT>.LOG.txt
...
// 1. \epoc32\release\wins\<VARIANT>\T_IMCM02C1.exe can be used at the command prompt
// or executable can be run from Windows Explorer.
// All other platform running instructions:
// 1. Copy \epoc32\release\<PLATFORM>\<VARIANT>\T_IMCM02C1.exe onto the other platform
...
// <DRIVE>:\system\libs on the other platform
// 4. Run T_IMCM02C1.exe on the other platform
// 
//
...
_LIT(KImcm02Test, "T_IMCM02c1 - CImEmailOperation Tests");
...
#define KOkFileNameEntryStructure _L("T_IMCM02c1_Entry_Structure.txt")
...
LOCAL_C void DoAddAttachmentL()
	...
	if (testActive->iStatus.Int())
		{
		testUtils->WriteComment(_L("Do AttachmentL failed"));
		testUtils->TestHarnessFailed(testActive->iStatus.Int());
		}
	...
	RFile file,file1;
	file.Open(fs,KAttachment1,EFileWrite);
	file1.Open(fs,KAttachment2,EFileWrite);

	testUtils->iMsvEntry->SetEntryL(messageId);
	CMsvAttachment* attachmentInfo = CMsvAttachment::NewL(CMsvAttachment::EMsvFile);
	CleanupStack::PushL(attachmentInfo);
	imEmailMessage->AttachmentManager().AddAttachmentL(file, attachmentInfo, testActive->iStatus);
...
LOCAL_C void doMainL()
	...
	//Create Forwarded message with attachment which is not complete
	testUtils->WriteComment(_L("Test 1 - Create Forward to Plaintext message with incomplete attachment Tests"));
	TMsvId messageWithIncompleteAttachment = CreateNewPlaintextMessageWithIncompleteAttachmentL();
	TRAPD(ret,DoPlaintextCreateForwardTestsL(messageWithIncompleteAttachment));
	if (ret)
		{
	...
	test.Printf(_L("DoAddAttachmentL Test"));
	//DoAddAttachment Tests
	testUtils->WriteComment(_L("DoAddAttachmentL"));
	TRAPD(err,DoAddAttachmentL());
	if (err)
		{
		testUtils->TestFinish(testNo,err);
		testUtils->TestHarnessFailed(err);
		delete messageSelection;
		Closedown();
		return;
		}
...
//
	...
	testUtils->WriteComment(_L("Test 2 - Create Forward to Plaintext message with no attachments Tests"));
	TRAP(ret,DoHtmlCreateForwardTestsL(messageSelection->At(5)));
	if (ret)
		{
		testUtils->TestFinish(testNo,ret);
		testUtils->TestHarnessFailed(ret);
	...
	testUtils->WriteComment(_L("Test 3 - Create Forward to Plaintext message with attachment Tests"));
	TRAP(ret,DoHtmlCreateForwardTestsL(messageSelection->At(1)));
	if (ret)
		{
		testUtils->TestFinish(testNo,ret);
		testUtils->TestHarnessFailed(ret);
...
GLDEF_C TInt E32Main()
	{	
	__UHEAP_MARK;
	test.Start(_L("T_IMCM02c1 Test CImEmailOperation class\n"));
