...
//
// Description:
// Name of test harness: T_IMCM02E2
// Component: IMCM
// Owner: KP
...
// \MSG\IMCMTSRC\T_IMCM02E2_Entry_Structure.txt	<DRIVE>:\msgtest\IMCM\T_IMCM02E2_Entry_Structure.txt
...
// <DRIVE>:\msglogs\T_IMCM02E2\Entry_Structure.txt
// Output files produced by running test harness:
// <DRIVE>:\msglogs\T_IMCM02E2.<PLATFORM>.<VARIANT>.LOG.txt
...
// 1. \epoc32\release\wins\<VARIANT>\T_IMCM02E2.exe can be used at the command prompt
// or executable can be run from Windows Explorer.
// All other platform running instructions:
// 1. Copy \epoc32\release\<PLATFORM>\<VARIANT>\T_IMCM02E2.exe onto the other platform
...
// <DRIVE>:\system\libs on the other platform
// 4. Run T_IMCM02E2.exe on the other platform
// 
//
...
_LIT(KImcm02Test, "T_IMCM02e2 - CImEmailOperation Tests");
...
#define KOkFileNameEntryStructure _L("T_IMCM02e2_Entry_Structure.txt")
...
LOCAL_C void SetDefaultServiceL(TMsvId smtpService)
	...
	if (smtpAccount.iSmtpAccountId != defaultAccount.iSmtpAccountId)
		{
...
LOCAL_C void doMainL()
	...
	//Create Receipt tohtml message with attachment and no text alternative to the HTML
	testUtils->WriteComment(_L("Test 1 - Create Receipt to HTML message with atcmt and no text alt. Tests"));
	TRAPD(ret,DoHtmlCreateReceiptTestsL(messageSelection->At(3)));
	if (ret)
		{
	...
	testUtils->WriteComment(_L("Test 2 -  Create Receipt to Plaintext message with incomplete atcmt Tests"));
	messageWithIncompleteAttachment = CreateNewPlaintextMessageWithIncompleteAttachmentL();
	TRAP(ret,DoHtmlCreateReceiptTestsL(messageWithIncompleteAttachment));
	if (ret)
		{
		testUtils->TestHarnessFailed(ret);
		delete messageSelection;
...
GLDEF_C TInt E32Main()
	{	
	__UHEAP_MARK;
	test.Start(_L("T_IMCM02e2 Test CImEmailOperation class\n"));
