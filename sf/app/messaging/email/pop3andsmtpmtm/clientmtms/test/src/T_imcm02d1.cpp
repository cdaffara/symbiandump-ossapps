...
//
// Description:
// Name of test harness: T_IMCM02D1
// Component: IMCM
// Owner: KP
...
// \MSG\IMCMTSRC\T_IMCM02d1_Entry_Structure_wins.txt	<DRIVE>:\msgtest\IMCM\T_IMCM02d1_Entry_Structure_wins.txt
// In addition the following is required for ARMI, ARM4, THUMB:
// \MSG\IMCMTSRC\T_IMCM02D1_Entry_Structure.txt			<DRIVE>:\msgtest\IMCM\T_IMCM02D1_Entry_Structure.txt
// Intermediate files produced while running test harness:
// <DRIVE>:\msglogs\T_IMCM02D1\Entry_Structure.txt
// Output files produced by running test harness:
// <DRIVE>:\msglogs\T_IMCM02D1.<PLATFORM>.<VARIANT>.LOG.txt
...
// 1. \epoc32\release\wins\<VARIANT>\T_IMCM02D1.exe can be used at the command prompt
// or executable can be run from Windows Explorer.
// All other platform running instructions:
// 1. Copy \epoc32\release\<PLATFORM>\<VARIANT>\T_IMCM02D1.exe onto the other platform
...
// <DRIVE>:\system\libs on the other platform
// 4. Run T_IMCM02D1.exe on the other platform
// 
//
...
_LIT(KImcm02Test, "T_IMCM02d1 - CImEmailOperation Tests");
...
#define KOkFileNameEntryStructure _L("T_IMCM02d1_Entry_Structure.txt")
...
LOCAL_C TMsvId CreateNewPlaintextMessageWithIncompleteAttachmentL()
	...
	_LIT(KAttachmentName1, "rfc82201.txt");
	attachmentInfo->SetAttachmentNameL(KAttachmentName1());
	...
	CleanupStack::PushL(attachmentInfo);
	_LIT(KAttachmentName2, "rfc82202.txt");
	attachmentInfo->SetAttachmentNameL(KAttachmentName2());
...
LOCAL_C void doMainL()
	...
	testUtils->WriteComment(_L("Test 1 - Create Forward As Atcmt to Plaintext message with incomplete atcmt Tests"));
	TMsvId messageWithIncompleteAttachment = CreateNewPlaintextMessageWithIncompleteAttachmentL();
	TRAPD(ret,DoPlaintextCreateForwardAsAttachmentTestsL(messageWithIncompleteAttachment));
	if (ret)
		{
...
//
//	Create HTML Forward As Attachment tests
//
	SetKnownDateAndTime();

	test.Console()->SetPos(3, 9);
	test.Printf(_L("Create Forward Tests 2:"));
	testUtils->WriteComment(_L("Test 2 - Create HTML Forward Tests"));
	...
	TRAP(ret,DoHtmlCreateForwardAsAttachmentTestsL(messageSelection->At(5)));
	if (ret)
		{
		testUtils->TestFinish(testNo,ret);
		testUtils->TestHarnessFailed(ret);
	...
	//Create Forward As Attachment plaintext message with attachment
	testUtils->WriteComment(_L("Test 3 - Create Forward As Atcmt to Plaintext message with atcmt Tests"));
	TRAP(ret,DoHtmlCreateForwardAsAttachmentTestsL(messageSelection->At(1)));
	if (ret)
		{
		testUtils->TestFinish(testNo,ret);
		testUtils->TestHarnessFailed(ret);
...
GLDEF_C TInt E32Main()
	{
	__UHEAP_MARK;
	test.Start(_L("T_IMCM02d1 Test CImEmailOperation class\n"));
