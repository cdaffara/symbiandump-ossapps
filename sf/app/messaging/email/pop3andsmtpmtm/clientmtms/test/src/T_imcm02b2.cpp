...
//
// Description:
// Name of test harness: T_IMCM02B2
// Component: IMCM
// Owner: KP
...
// \MSG\IMCMTSRC\T_IMCM02B2_Entry_Structure.txt	<DRIVE>:\msgtest\IMCM\T_IMCM02B2_Entry_Structure.txt
...
// <DRIVE>:\msglogs\T_IMCM02B2\Entry_Structure.txt
// Output files produced by running test harness:
// <DRIVE>:\msglogs\T_IMCM02B2.<PLATFORM>.<VARIANT>.LOG.txt
...
// 1. \epoc32\release\wins\<VARIANT>\T_IMCM02B2.exe can be used at the command prompt
// or executable can be run from Windows Explorer.
// All other platform running instructions:
// 1. Copy \epoc32\release\<PLATFORM>\<VARIANT>\T_IMCM02B2.exe onto the other platform
...
// <DRIVE>:\system\libs on the other platform
// 4. Run T_IMCM02B2.exe on the other platform
// 
//
...
_LIT(KImcm02Test, "T_IMCM02b2 - CImEmailOperation Tests");
...
#define KOkFileNameEntryStructure _L("T_IMCM02b2_Entry_Structure.txt")
...
LOCAL_C void doMainL()
	...
	test.Console()->SetPos(3, 13);
	test.Printf(_L("Create Reply Tests 1:  "));
	//Create reply to html message with attachment and no text alternative to the HTML
	testUtils->WriteComment(_L("Test 1 - Create Reply to HTML message with attachment and no text alt. Tests"));
	TRAPD(ret,DoHtmlCreateReplyTestsL(messageSelection->At(3)));
	if (ret)
		{
	...
	test.Console()->SetPos(3, 14);
	test.Printf(_L("Create Reply Tests 2:  "));
	//Create message with attachment which is not complete
	testUtils->WriteComment(_L("Test 2 - Create Reply to HTML message with incomplete attachment Tests"));
	TMsvId messageWithIncompleteAttachment = CreateNewPlaintextMessageWithIncompleteAttachmentL();
	TRAP(ret,DoHtmlCreateReplyTestsL(messageWithIncompleteAttachment));
	if (ret)
		{
		testUtils->TestHarnessFailed(ret);
		delete messageSelection;
